...
class Solution {
  public:
  void inorder(Node *root,Node *&prev ,Node *&found)
  {
      if(root == NULL)
      return ;
      inorder(root->left,prev,found);
      if(found != NULL)
      {
      ...
      if(prev != NULL && root->data < prev->data && found == NULL){
      found = prev;
      prev = root;
      }
      else
      prev = root;
      inorder(root->right,prev,found);
  }
    struct Node *correctBST(struct Node *root) {
        // code here
        if(root == NULL)
        return root;
        Node *prev = NULL,*found = NULL;
        inorder(root,prev,found);
        swap(prev->data,found->data);
        return root;
    }
};
